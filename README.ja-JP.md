...
- 双方向選択機能をサポートし、クリック＆ドラッグでのセル選択や Cmd/Ctrl+C での TSV 形式でのコピーが可能
...
6. JSON/YAML/TOML/XML のプレビューではキーをクリックしてソース行へジャンプでき、CSV/TSV は先頭行・先頭列の固定、セルクリックでのロケート、スクロール追従に対応した表形式で表示されます
