...
- 任務列表勾選回寫
- 代碼高亮同埋複製掣
- 懸停 Markdown 表格時可以複製為 Markdown、ASCII 或 TSV
...
- 多儲存格選中時喺選區右上方提供分體複製操作：主按鈕複製為 TSV，下拉選單提供「複製為 ASCII」同「複製為 Markdown」
...
6. Markdown 預覽可以喺懸停表格時複製為 Markdown、ASCII 或 TSV；JSON / JSONL / YAML / TOML / XML 可以點擊鍵名跳返去源碼對應嗰行，並提供焦點態高亮；CSV / TSV 會以表格方式預覽，支援首行首列固定、點擊儲存格定位、焦點態高亮、選區右上方嘅 TSV 主複製按鈕及 ASCII/Markdown 下拉操作同 `Cmd/Ctrl+C` 複製
