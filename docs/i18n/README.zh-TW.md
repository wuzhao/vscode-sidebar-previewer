...
VS Code 的側邊欄預覽器，支援實時渲染、滾動同步、樹形檢視、縮放控制等功能，多檔案格式支援包括 Markdown、LaTeX、Mermaid、JSON、JSONL、YAML、TOML、XML、CSV 和 TSV。
...
- 支援單行標籤和格式化結構化值的 Front Matter 屬性表格
- GitHub Alert 提示塊渲染
- 帶懸停目錄的 Skeleton Outline
- 任務列表勾選回寫
- 表格儲存格內的互動式任務列表核取方塊
- 程式碼高亮、複製與自動換行按鈕
- 懸停 Markdown 表格時，可複製為 Markdown，或複製為 ASCII、TSV、CSV
...
- 多儲存格選取時在選區右上方提供分割複製操作：主按鈕複製為 TSV，下拉選單提供 Markdown、ASCII、TSV 和 CSV 格式
...
1. 開啟任意支援的檔案（`.md`、`.markdown`、`.tex`、`.katex`、`.latex`、`.mmd`、`.mermaid`、`.json`、`.jsonc`、`.jsonl`、`.yml`、`.yaml`、`.toml`、`.xml`、`.csv`、`.tsv`）
2. 點選左側 Activity Bar 中的 Sidebar Previewer 圖示
3. 預覽面板會自動顯示當前檔案的渲染結果
4. 使用工具欄或 `Cmd/Ctrl` + 滑鼠滾輪進行縮放
5. Mermaid 預覽支援拖拽檢視放大區域
6. Markdown 預覽可在懸停表格時複製為 Markdown、ASCII、TSV 或 CSV；JSON / JSONL / YAML / TOML / XML 可點選鍵名跳轉到原始碼對應行並提供焦點狀態高亮；CSV / TSV 預覽為表格檢視，支援首行首列固定、點選儲存格定位、焦點狀態高亮、選區右上方的 TSV 主複製按鈕及 Markdown/ASCII/TSV/CSV 下拉操作與 `Cmd/Ctrl+C` 複製
